...
<template>
  ...
  <div class="w-full h-fit border rounded-xl flex flex-col cursor-pointer" @click="()=>{
    router.push({name: 'room-detail', params: {roomId: props.id}})
  }">
    <img
    ...
    <div class="flex flex-col w-full p-3">
      ...
      <div class="text-gray-500">Costo: {{ cost }}</div>
      <div class="italic">{{ category }}</div>
      <div>Numero posti: {{ number_of_seats }}</div>
      <div>Disponibile: {{ is_available ? "Si" : "No" }}</div>
    </div>
  </div>
</template>
