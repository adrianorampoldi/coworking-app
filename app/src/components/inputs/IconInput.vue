<script setup lang="ts">
  ...
  const props = defineProps({
    id: { type: String, default:"icon-input", required: true },
...
<template>
  <div class="p-1.5 flex flex-row w-90 max-w-full">
    ...
    <!-- Collegare inputValue in maniera bilaterale all'input -->
    <!-- aggiungere id, type, placeholder -->
    <input v-model="inputValue" :id="id" :type="type" :placeholder="placeholder" class="flex-1 font-instrument focus:outline-none text-center" />
  </div>
</template>
